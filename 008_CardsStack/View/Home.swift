...
struct Home: View {
    ...
    var body: some View {
		...
		.onAppear {
			for index in 1...5 {
				messages.append(Message(imageFile: "\(index)"))
			}
		}
    }
}
...
struct SwipeCarousel<Content: View, ID, Item>: View where Item: RandomAccessCollection, Item.Element: Equatable & Identifiable, ID: Hashable{
